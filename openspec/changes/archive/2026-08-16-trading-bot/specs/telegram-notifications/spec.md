...
The system SHALL send one daily digest at the end of each run summarizing P&L for the day, open positions, risk status, and the evaluation-gate countdown.

#### Scenario: Digest after successful run
- **WHEN** the daily pipeline completes
...
#### Scenario: Digest after run failure
- **WHEN** the daily pipeline fails before report generation
- **THEN** a Telegram alert is sent stating the run failed (so silence never looks like success)

### Requirement: Delivery reliability
The system MUST retry a failed Telegram send once, and MUST log the failure into the daily report if the retry also fails.
...
- **THEN** the failure is recorded in the daily report and the pipeline continues (no crash)
